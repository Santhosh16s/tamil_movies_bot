...
async def handle_resolution_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.message.reply_text(
            "⚠️ **File வரவில்லையா?** இந்தக் கோப்பைப் பெற, கீழே உள்ள **பட்டனைக் கிளிக் செய்து**, எனது **Private chat இல் `/start` என்று  அனுப்பவும்** மீண்டும் உங்களுக்குத் தேவையான **quality ஐ கிளிக் செய்யவும்.**",
            parse_mode="Markdown",
    ...
    if payload and payload.startswith("sendfile_"):
        try:
            ...
            if file_id_to_send:
                ...
                sent_msg = await context.bot.send_document(
                    chat_id=update.effective_chat.id,  # message வந்த இடத்தில் file போடுவோம்
                    document=file_id_to_send,
                    caption=caption,
                    parse_mode="HTML"
                )
