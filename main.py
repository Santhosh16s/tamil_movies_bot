...
async def handle_resolution_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if file_msg_id_to_send and file_store_channel:
            ...
            # கோப்பை பட்டன் கிளிக் செய்யப்பட்ட குழுவிலேயே அனுப்பவும்
            target_chat_id = query.message.chat.id # <--- இந்த வரி மாற்றப்பட்டது
            
            sent_msg = await context.bot.copy_message(
                chat_id=target_chat_id, # <--- இந்த வரி மாற்றப்பட்டது
                from_chat_id=file_store_channel, 
                message_id=file_msg_id_to_send, 
                caption=caption,
                parse_mode="HTML"
            )
            logging.info(f"File sent successfully to chat {target_chat_id} for movie '{movie_name_key}' resolution '{res}'.")
            # உறுதிப்படுத்தல் மெசேஜ் (புதிய வரி)
            await query.message.reply_text("✅ கோப்பு இந்த குழுவிலேயே அனுப்பப்பட்டது.")
            
            # குழு chat ஐ சுத்தமாக வைத்திருக்க, தாமதத்திற்குப் பிறகு நீக்கவும் (புதிய வரி)
        ...
        logging.error(f"❌ கோப்பு அனுப்ப பிழை: {e}", exc_info=True) 
