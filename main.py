...
async def start_with_payload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    payload = context.args[0] if context.args else None # /start கட்டளைக்குப் பின் வரும் payload-ஐப் பெறுகிறது

    # முதல் முறை /start செய்யும் பயனரை பதிவு செய்க (உங்கள் track_user செயல்பாடு இருந்தால்)
    # await track_user(user) # track_user செயல்பாட்டை இங்கேயும் அழைக்கலாம்.

    if payload and payload.startswith("send_"):
        # பேலோடில் இருந்து திரைப்படம் மற்றும் resolution விவரங்களைப் பிரிக்கவும்
        try:
            _, movie_name_key, res = payload.split("_", 2) # e.g., "send_MovieName_720p" -> "send", "MovieName", "720p"
            
            movie = movies_data.get(movie_name_key)
            if not movie:
                await update.message.reply_text("❌ மன்னிக்கவும், இந்தத் திரைப்படம் எங்கள் Database-இல் இல்லை அல்லது தகவல்கள் முழுமையாயில்லை.")
                return
            ...
            if file_id_to_send:
                ...
                sent_msg = await context.bot.send_document(
                    chat_id=user.id, # பயனரின் தனிப்பட்ட சாட்டிற்கு அனுப்புகிறது
                    document=file_id_to_send,
                    caption=caption,
                    parse_mode="HTML"
                )
        ...
        # வழக்கமான /start மெசேஜ் (பேலோடு இல்லாதபோது அல்லது பேலோடு send_ எனத் தொடங்காதபோது)
        await update.message.reply_text(f"வணக்கம் {user.first_name}! 👋\n\n"
                                        "🎬 லேட்டஸ்ட் 2025 HD தமிழ் படங்கள் வேண்டுமா? ✨\n"
                                        "விளம்பரமில்லா உடனடி தேடலுடன், தரமான சினிமா அனுபவம் இங்கே! 🍿\n\n"
                                        "🎬 தயவுசெய்து திரைப்படத்தின் பெயரை டைப் செய்து அனுப்புங்கள்!")
